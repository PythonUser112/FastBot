...
int mot_right_1 = 5;
int mot_right_2 = 4;
int mot_left_korrektur = 15; // Justieren, um bessere Korrektur beim Geradeausfahren zu erreichen.
...
void get_distance_cm() {
  ...
  millisekunden_seit_start = millisekunden_seit_start + zeit / 1000;
}
...
void noch_langsamer_fahren() {
  anhalten();
  analogWrite(mot_left_1, 100 - mot_left_korrektur);
  analogWrite(mot_right_1, 100 - mot_right_korrektur);
}

void langsam_drehen_links() {
  anhalten();
  digitalWrite(mot_right_2, HIGH);
  analogWrite(mot_left_1, 175);
  analogWrite(mot_right_1, 175);
}
...
void setup() {
  ...
  lcd.setCursor(0, 1);
  lcd.print("Taster.");
  while(digitalRead(taster_pin) == 1) {
  }
  lcd.clear();
  lcd.noBacklight();
  vorwaerts();
...
// Funktion wie delay, führt aber Lichtersteuerung aus
...
void kollisionsbewahrung() {
  ...
  get_distance_cm();
  if(entfernung_links > entfernung) {
...
void lichtersteuerung() {
  ...
  // Blinkersteuerung
  if(lichter > 0 && millisekunden_seit_start - letzter_blinker_wechsel > 500) {
